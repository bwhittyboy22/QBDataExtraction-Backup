...
<#
This script is hard coded to run the daily quickbooks extract manually. 
It will not upload files to the Data Lake. It only does prior day and 
always includes line items 
#>
...
$baseReportDestinationPath = "C:\Users\BenjaminW.admin\Documents\QBExractions\daily"
$reportPath = Join-Path -Path $baseReportDestinationPath -ChildPath "$currentYear\$currentMonth\$currentDay"
...
foreach ($division in $divisionsToProcess) {
    ...
    foreach ($reportType in $reportsToProcess) {
        try {
            ...
            $OutputPath = Join-Path -Path $reportPath -ChildPath "$($division.Division)_$($reportType)_${CurrentDateTimeForExportFileName}.xml"
