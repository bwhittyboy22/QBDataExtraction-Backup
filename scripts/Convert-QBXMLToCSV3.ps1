...
function ConvertTo-FlatRecord {
    ...
    foreach ($child in $Node.ChildNodes) {
        ...
        if (-not $Prefix -and (Test-IsLineNode $childName)) { continue }

        $childPath = if ($Prefix) { "$Prefix.$childName" } else { $childName }
        ...
        elseif ($child.HasChildNodes) {
            ...
            if (-not $Acc.Contains($childPath)) { $Acc[$childPath] = '' }
        }
    }
...
function Get-RecordKey {
    ...
    foreach ($k in 'TxnID', 'ListID') {
        if ($Record.Contains($k) -and $Record[$k]) { return $Record[$k] }
    }
    return $null
}
...
function Resolve-SchemaColumns {
    ...
    if (-not $SchemaRoot -or -not (Test-Path $SchemaRoot)) { return $null }
    ...
    foreach ($name in $CandidateNames) {
        ...
        if ($hit) {
            ...
            if (-not ($json.PSObject.Properties.Name -contains 'Fields')) { continue }
...
function Write-Csv {
    ...
    try {
        $sw.WriteLine(($Columns | ForEach-Object { Format-CsvField $_ $Delim }) -join $Delim)
        foreach ($row in $Rows) {
            $line = foreach ($c in $Columns) {
                $v = if ($row.Contains($c)) { [string]$row[$c] } else { '' }
...
function Add-Row {
    ...
    foreach ($k in $Row.Keys) {
        if (-not $Section.Keys.Contains($k)) { $Section.Keys[$k] = $true }
    }
}
...
function Add-RecordToSections {
    ...
    foreach ($child in $RecordNode.ChildNodes) {
        if ($child.NodeType -ne [System.Xml.XmlNodeType]::Element) { continue }
        $childName = $child.LocalName
        if (-not (Test-IsLineNode $childName)) { continue }
        ...
        if ($lineName -like '*LineGroupRet') {
            ...
            foreach ($gc in $child.ChildNodes) {
                if ($gc.NodeType -ne [System.Xml.XmlNodeType]::Element) { continue }
                $gcName = $gc.LocalName
                if (Test-IsLineNode $gcName) { continue }   # nested lines handled below
                $sub = ConvertTo-FlatRecord -Node $gc -Prefix $gcName
                foreach ($kv in $sub.GetEnumerator()) { $groupFlat[$kv.Key] = $kv.Value }
            }
            ...
            foreach ($kv in $groupSelf.GetEnumerator()) {
                if (-not $groupFlat.Contains($kv.Key)) { $groupFlat[$kv.Key] = $kv.Value }
            }
            Add-Row -Section $Sections[$secKey] -Row $groupFlat
            $groupLineId = if ($groupFlat.Contains('TxnLineID')) { $groupFlat['TxnLineID'] } else { $null }

            # nested lines within the group
            foreach ($nested in $child.ChildNodes) {
                if ($nested.NodeType -ne [System.Xml.XmlNodeType]::Element) { continue }
                $nestedName = $nested.LocalName
                if (-not (Test-IsLineNode $nestedName)) { continue }
                ...
                foreach ($kv in $flat.GetEnumerator()) { $row[$kv.Key] = $kv.Value }
    ...
    if ($records.Count -eq 0) {
        ...
        foreach ($n in $allRet) {
            ...
            while ($p -and $p.NodeType -eq [System.Xml.XmlNodeType]::Element) {
                if ($p.LocalName -like '*Ret') { $hasRetAncestor = $true; break }
                $p = $p.ParentNode
            }
            if (-not $hasRetAncestor) { $records.Add($n) }
        }
    }
    return $records
}
    ...
    foreach ($secName in ($sections.Keys | Sort-Object)) {
        ...
        if (-not $sec.IsParent) {
            foreach ($lc in 'ParentTxnID', 'GroupTxnLineID') {
                if ($discovered -contains $lc) { $linkCols += $lc }
            }
        }

        if ($usedSchema) {
            $ordered = [System.Collections.Generic.List[string]]::new()
            foreach ($c in $linkCols) { if (-not $ordered.Contains($c)) { $ordered.Add($c) } }
            foreach ($c in $schemaCols) { if (-not $ordered.Contains($c)) { $ordered.Add($c) } }
            if (-not $StrictSchema) {
                ...
                if ($extra.Count -gt 0) {
                    ...
                    foreach ($c in $extra) { $ordered.Add($c) }
                }
            }
        ...
        $summaries += [pscustomobject]@{
            ...
            Section = $sec.Name
            Type    = if ($sec.IsParent) { 'parent' } else { 'line' }
            Rows    = $sec.Rows.Count
            Columns = $columns.Count
            Schema  = if ($usedSchema) { 'yes' } else { 'auto' }
...
# Output folder.
if (-not $OutputPath) {
    $inRoot = if ($item.PSIsContainer) { $item.FullName } else { $item.Directory.FullName }
...
if (-not $SchemaFolderPath) {
    ...
    foreach ($c in $candidates) {
        if (Test-Path $c) { $SchemaFolderPath = (Resolve-Path $c).Path; break }
    }
}
