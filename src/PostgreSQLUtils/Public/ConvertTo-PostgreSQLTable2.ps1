function ConvertTo-PostgreSQLTable2 {
    ...
    begin {
        ...
        # Format MDUploadDate if provided
        if ($MDUploadDate) {
            try {
                $formattedDate = [DateTime]::ParseExact($MDUploadDate, 'yyyy-MM-dd', $null).ToString('yyyy-MM-dd 00:00:00.000000')
            } catch {
                throw "Invalid date format for MDUploadDate. Expected format: yyyy-MM-dd"
            }
        }
    }
...
"@
            ...
            $columns = if ($TableSchema -eq "public") {
                ...
                $baseColumns + @(
                    """md_is_current"" BOOLEAN DEFAULT TRUE",
                    """md_upload_date"" TIMESTAMP DEFAULT $(
                        if ($MDUploadDate) {
                            "'$formattedDate'"
                        } else {
                            "CURRENT_TIMESTAMP"
