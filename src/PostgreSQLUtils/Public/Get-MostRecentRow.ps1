function Get-MostRecentRow {
    ...
    process {
        try {
            ...
            $dateColumnExists = Test-ColumnExists -TableName $TableName -ColumnName $DateColumn -TableSchema $TableSchema
            if (-not $dateColumnExists) {
                throw "Date column '$DateColumn' does not exist in table '$TableSchema.$TableName'."
            }

            # Set the PGPASSWORD environment variable
            $env:PGPASSWORD = $pgConfig.Password

            # SQL query to fetch column headers and most recent row data
            $sql = @"
SELECT *
